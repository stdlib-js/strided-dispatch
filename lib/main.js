...
function dispatch( fcns, types, data, nargs, nin, nout ) {
	...
	if ( isFunction( fcns ) ) {
		...
		throw new TypeError( format( '0gn5s', fcns ) );
	}
	if ( !isCollection( types ) ) {
		throw new TypeError( format( '0gn3A', types ) );
	}
	...
	if ( !isPositiveInteger( nargs ) ) {
		throw new TypeError( format( '0gn5u', nargs ) );
	}
	if ( !isNonNegativeInteger( nin ) ) {
		throw new TypeError( format( '0gn5v', nin ) );
	}
	if ( !isNonNegativeInteger( nout ) ) {
		throw new TypeError( format( '0gn5w', nout ) );
	}
	narrays = nin + nout;
	if ( narrays === 0 ) {
		throw new Error( format( '0gn1V' ) );
	}
	if ( fcn ) {
		nfcns = types.length / narrays;
		if ( !isInteger( nfcns ) ) {
			throw new Error( format( '0gn1W' ) );
		}
	} else {
		nfcns = fcns.length;
	...
	*/
	function dispatcher() {
		...
		if ( argc !== nargs ) {
			if ( argc < nargs ) {
				throw new Error( format( '0gn0l' ) );
			}
			throw new Error( format( '0gn0m' ) );
		}
		N = arguments[ 0 ];
		if ( !isInteger( N ) ) {
			throw new TypeError( format( '0gn7w', N ) );
		}
		shape = [ N ];
		...
		if ( idx < 0 ) {
			throw new TypeError( format( '0gn1S' ) );
		}
